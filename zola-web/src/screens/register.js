...
function ZaloRegistration() {
    ...
    switch (name) {
      ...
      case 'phoneNumber':
        if (!/^\d{10}$/.test(value)) {
          ...
          try {
            const response = await fetch(`http://localhost:3001/account/check-phone?phoneNumber=0${value.slice(-9)}`);
  ...
  const handleRegister = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:3001/account/addAccountWeb', {
        method: 'POST',
